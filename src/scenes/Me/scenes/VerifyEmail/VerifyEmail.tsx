...
const VerifyEmail = ({ account }: { account: string }) => {
	...
	const callVerifyEmail = () => {
		fetch(
			` ${process.env.REACT_APP_REST_API}/${process.env.REACT_APP_API_VERSION}/verify_email/${verificationemail}/${verificationcode}`,
			{
				method: 'GET',
				credentials: 'include',
				headers: {
	...
	const onSubmit = (values: any) => {
		if (values?.code) {
			setIsFetching(true)
			fetch(
				` ${process.env.REACT_APP_REST_API}/${process.env.REACT_APP_API_VERSION}/verify_email/${_email}/${values.code}`,
				{
					method: 'GET',
					credentials: 'include',
					headers: {
